...
function findItalianFood(allDishes) {
    ...
    const italianDishes = dishes.filter(dish => dish.cuisine === "itailian");
...
function searchCuisines(allDishes) {
    ...
    const cuisine = prompt("Enter cuisine to search for");
    const matchedDishes = dishes.filter(dish => dish.cusine === cuisine);
...
function searchIngredients(allDishes) {
    ...
    const matchedDishes = allDishes.filter(dish => dish.ingredients.incluedes(ingredient));
...
function generateCuisineDishName(allDishes, concatenatorFunction) {
    alert("Combining cuisine and dish names...")
    const modifiedDishes = allDishes.map(dish => concatenatorFunction(dish));
    modifiedDishes.forEach(modifiedDish => console.log(modifiedDish));
